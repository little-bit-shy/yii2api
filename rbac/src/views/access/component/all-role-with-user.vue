    ...
    export default {
        ...
        methods: {
            ...
            getAllPermissionsWithRoleData(timeout = 1) {
                return new Promise((resolve) => {
                    setTimeout(() => {
                        ...
                        (new ajax()).send(this,'/account/auth-item/all-role-with-user', {
                            'user_id': this.userId
                        }).then((response) => {
                            var data = response.data;
            ...
            onChange(type, name) {
                switch (type) {
                    case true:
                        (new ajax()).send(this,'/account/auth-item/add-user-role', {
                            'role': name,
                            'user_id': this.userId,
                        }).then((response) => {
                            var data = response.data;
                        ...
                        break;
                    case false:
                        (new ajax()).send(this,'/account/auth-item/delete-user-role', {
                            'role': name,
                            'user_id': this.userId,
                        }).then((response) => {
                            var data = response.data;
