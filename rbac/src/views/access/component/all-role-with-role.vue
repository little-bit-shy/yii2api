    ...
    export default {
        ...
        methods: {
            ...
            getAllPermissionsWithRoleData(timeout = 1) {
                return new Promise((resolve) => {
                    setTimeout(() => {
                        (new ajax()).send(this,'/account/auth-item/all-role-with-role', {
                            'role': this.role
                        }).then((response) => {
                            var data = response.data;
            ...
            onChange(type, name) {
                switch (type) {
                    case true:
                        (new ajax()).send(this,'/account/auth-item/add-role-role', {
                            'role': this.role,
                            'child_role': name,
                        }).then((response) => {
                            var data = response.data;
                        ...
                        break;
                    case false:
                        (new ajax()).send(this,'/account/auth-item/delete-role-role', {
                            'role': this.role,
                            'child_role': name,
                        }).then((response) => {
                            var data = response.data;
